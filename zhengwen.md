...
该标准基于在2.4GHz频带中以250kbps运行的IEEE 802.15.4
[\[IEEE802154\]](https://standards.ieee.org/findstds/standard/802.15.4-2006.html)
PHY（物理）和MAC（媒体访问控制）层。Thread协议栈采用的是IEEE 802.15.4-2006版本。

所述802.15.4 MAC层用于基本的消息处理和拥塞控制。MAC层包括设备用于监听干净信道的CSMA
（Carrier Sense Multiple Access，载波侦听多路访问）机制，
...
Thread网络通常最多有32个活动路由器，它们使用基于设备路由表的消息的下一跳路由（next-hop routing）。
设备路由表由协议栈维护，以确保所有路由器都具有Thread网络中任何其他路由器的连接和最新路径。
使用RIP（路由信息协议）算法（来自
[\[RFC 1058\]](https://www.ietf.org/rfc/rfc1058)和[\[RFC 2080\]](https://www.ietf.org/rfc/rfc2080)但未采用其特定的消息格式）。
所有路由器使用一种压缩格式的MLE（Mesh Link Establishment）实现路由器间路由成本消息的交互。

**注意：**从IP的角度看，Thread网络支持路由器和主机。主机是指休眠设备或REED。

## MLE消息

MLE消息（参阅[\[draft-kelsey-intarea-mesh-link-establishment-06\]](https://datatracker.ietf.org/doc/draft-kelsey-intarea-mesh-link-establishment/)
对Thread协议规范第4章消息链路的建立，所做的拓展）被用于建立和配置安全的无线链路，检测相邻设备，及维护Thread的网络设备之间的路由的开销。
Thread网络使用单跳链路本地单播（single-hop link local unicasts）和路由器之间多播（multicasts between Routers）来传输MLE消息。

根据拓扑和物理环境的变化，MLE消息用于识别，配置和保护邻近设备的链路。MLE还用于分发跨Thread网络共享的配置值，如通道（channel）
和PAN（Personal Area Network，个人区域网络）ID。这些消息可以由MPL
（有关详细信息，请参阅[\[draft-ietf-roll-trickle-mcast-09\]](https://tools.ietf.org/html/draft-ietf-roll-trickle-mcast-09)）
（Multicast Protocol for Low power and Lossy Networks，低功耗和有损网络的多播协议）指定的简单洪泛转发（forwarded with simple flooding）。

MLE消息也确保了两个设备之间非对称链路路由开销的考虑。非对称链路成本在802.15.4网络中很常见。
为了确保双向通信是可靠的，考虑非堆成双向链路的成本是非常必要的。

## 路由发现和修复

在按需路由发现在经常用于低功耗802.15.4网络中。然而，由于路由发现请求洪泛网络，按需路由发现需要昂贵的网络和带宽开销。

在Thread网络中，所有路由器定期交换包含到所有邻居路由器链路成本信息的单跳MLE广播包，以及到Thread网络中所有其他路由器的路径开销。

通过这些定期的本地更新，所有路由器都具有Thread网络中任何其他路由器的最新路径开销信息，因此不需要按需路由发现。
如果路由不再可用，路由器可以在所有可达目的节点的最优候选路由中进行选择。
这种自愈路由机制允许路由器快速检测其他路由器是否已经脱离Thread网络，并计算最佳路径以维护与Thread网络中所有其他设备的连接。
...
[图4](#_bookmark24)Thread网络中链路质量和链路开销的示例。

![](/assets/各种链路成本实例.png)

**图4 Thread网络中链路质量和链路成本的示例**

Thread网络任一节点的链路成本是到达该节点所有链路成本中的最小值。路由器监控链路成本，当网络的无线链路质量或拓扑结构发生变化时，
通过使用周期性的MLE消息将新的链路开销通告整个Thread网络。路由开销是基于两个设备之间的双向链路质量。

举个简单的例子，假设有一个预先配置的共享加密数据的网络中，所有设备在同一时间上电。
每个路由器将周期性地向邻居节点广播初始链路开销数据。在每个路由器内部将存储未包含在广播中的邻节点信息。

前几个广播的路径成本等于链路成本，因为仅知道邻居路由的信息，如[图5](#_bookmark25)所示。

![](/assets/成型电后线程网络.png)

**图5.成型电中的Thread网络**

但是，随着路由器开始接收邻居节点的广播，其中包含两跳或更多跳的其他路由器链路开销；
其路由表填充了多跳路径成本，然后被转发到其他路由器，直到最终出现了网络中所有路由器之间的连接信息，
如所示[图6](#_bookmark26)和[图7](#_bookmark27)。
...
当路由器从邻居路由器接收到一个新的MLE的广播，无论是否已经存在该设备的邻居表项（不存在则添加）。
MLE广播中包含邻居路由器到该路由器的链路消耗及其他路由器的更新的路由信息，因此该路由设备根据上述信息更新其路由表。

可通过查看子设备的地址的高位来确定父路由器地址，从而实现到子设备的路由。
一旦设备知道父路由器，就具有该设备的路径开销信息和邻节点路由信息。

活动路由器的数量被限制在32以内，受限于可包含在单个802.15.4分组数据包中的路由和成本信息的量。
此限制为目前32个路由器但提供64活动的路由器地址，以允许路由器地址的循环利用。

## 路由

设备使用IP路由转发数据包。设备的路由表填充有压缩形式的用于路由的Mesh本地ULA地址和相应的邻节点信息。

距离矢量路由用于获取Thread网络中路由器地址的路由路径。当在Thread网络中进行路由时，该16位地址的高6位定义了目标路由器的路由器地址。
如果目的地址的较低位为0，则该路由器就是最终的目标节点。否则，目标路由器负责根据16位目标地址的低位将消息转发到最终目标节点。

为了实现Thread网络之外的路由，边界路由器通知Leader其所服务的特定前缀（ES），且该信息将包含在MLE消息中分发到整个Thread网络。
Thread网络数据包括：前缀数据，它是前缀本身，6LoWPAN上下文，边界路由器，及该前缀的DHCPv6服务器。
如果设备使用该前缀配置其IPv6地址，则使用SLAAC（无状态地址自动配置）或联系对应的DHCP（动态主机配置协议）服务器。
Thread网络数据还包括一个路由服务器列表，它们是默认的边界路由器的路由地址。

Leader被指定做出决策，选择REED成为路由器或允许路由器降级为REED。Leader也分配和管理路由器地址。
然而，此路由Leader中包含的所有信息都存在于其他路由器中，并且如果路由Leader变得无法访问，
则另一个路由器将被推举为Leader接管网络，此过程无需用户干预。

## 重试和确认

虽然在Thread协议栈中使用UDP传递消息，但仍然需要可靠的消息传递。这是使用一系列轻量级机制实现的，如下所示：

·MAC级重试：设备到设备的每一跳都使用MAC级确认，如果有一跳没有接收到MAC ACK消息，对应设备的MAC将重传该消息。

·应用级重试：应用程序级可以决定消息的可靠性是否是关键参数，并且在必要情况下可以实现其自己的重试机制。
